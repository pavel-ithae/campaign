...
namespace Campaign
{
    public class Layout : IDisposable, IEnumerable<Layout.EntryInfo>
    {
        ...
        public EntryInfo GetEntryInfo(int index)
        {
            ...
            return entryInfo;
#else
            LayoutAPI.GetEntryInfo(_layoutPtr, index, out var entryInfoNative).ValidateAPICall();

            try
            {
                return LayoutAPI.LayoutEntryInfoMarshaller.ConvertToManaged(entryInfoNative);
            }
            finally
            {
                LayoutAPI.LayoutEntryInfoMarshaller.Free(entryInfoNative);
            }
#endif
        }
